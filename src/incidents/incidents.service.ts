...
export class IncidentsService {
  ...
  async createIncident(dto: CreateIncidentDTO) {
    try{
      let incident = new Incident(EIncidentType[dto.type], dto.measurement);
      incident.mineSite = await this.minesiteService.getMineSiteById(dto.mineSite)
  ...
  async saveCombinedIncidents(dto: CombinedIncidentDTO) {
    const incidents: CreateIncidentDTO[] = [{ measurement: dto.temperature, type: EIncidentType.TEMPERATURE.toString(), mineSite: dto.origin },
    { measurement: dto.heatIndex, type: EIncidentType.HEATINDEX.toString(), mineSite: dto.origin }
      , { measurement: dto.humidity, type: EIncidentType.HUMIDITY.toString(), mineSite: dto.origin }];
    ...
    while (i < incidents.length) {
      ...
      if(createIncident.type == EIncidentType.TEMPERATURE){
        if(createIncident.measurement > 17){
          ...
          this.notificationService.notify('COMPANY',new CreateNotificationDTO(`${createIncident.mineSite.name} minesite is at its highest temperature`, 'COMPANY'), createIncident.mineSite.company.id )
          ...
          this.notificationService.notify('COMPANY',new CreateNotificationDTO(`${createIncident.mineSite.name} minesite is at a low temperature`, 'COMPANY'), createIncident.mineSite.company.id )
        }
        ...
        if(createIncident.measurement < 14){
          ...
          this.notificationService.notify('COMPANY',new CreateNotificationDTO(`${createIncident.mineSite.name} minesite is at a low humidity`, 'COMPANY'), createIncident.mineSite.company.id )
          ...
          this.notificationService.notify('COMPANY',new CreateNotificationDTO(`${createIncident.mineSite.name} minesite is at a high humidity`, 'COMPANY'), createIncident.mineSite.company.id )
        }else{
          await this.incidentRepo.update({
            id: createIncident.id
          }, {
